...
export const RadioButton = ({ type, radioType }) => {
  ...
  switch (radioType) {
    case 'payment':
      return <View style={styles.radio}>{type === paymentType ? <View style={styles.selected} /> : null}</View>
    case 'delivery':
      return <View style={styles.radio}>{type === deliveryType ? <View style={styles.selected} /> : null}</View>
  }
}
