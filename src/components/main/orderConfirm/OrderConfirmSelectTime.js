    ...
    <View>
      ...
      {deliveryType === type.later ? (
        <TouchableOpacity style={buttons.radioButton}>
          ...
          <Text onPress={() => toggleModal('date')} style={[inputs.inptTime, { width: '60%' }]}>
            {deliveryDay}
          </Text>
          <Text onPress={() => toggleModal('time')} style={inputs.inptTime}>
            {deliveryTime}
          </Text>
        </TouchableOpacity>
      ) : (
